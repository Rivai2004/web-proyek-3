...
if ($op == 'delete') {
  $id = $_GET['id'];
  $sql1 = "delete from slide_gambar where id= '$id'";
...
if ($op == 'edit') {
  $id = $_GET['id'];
  $sqldef = "select * from slide_gambar where id = '$id'";
...
if (isset($_POST['simpan'])) { //untuk create
  ...
  if ($judul && isset($_FILES['image']['name'])) {
    ...
    if (move_uploaded_file($image_tmp, $folder_tujuan)) {
      if ($op == 'edit') { //untuk update
        $sql1 = "update slide_gambar set image='$image',judul='$judul' where id = '$id'";
        $q1 = mysqli_query($conn, $sql1);
        if ($q1) {
        ...
        $sql1 = "insert into slide_gambar(image,judul) values ('$image','$judul')";
        $q1 = mysqli_query($conn, $sql1);
        if ($q1) {
